...
def run_em(x, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        # E-step
        for i in range(K):
            w[:, i] = np.exp(-0.5 * ((x-mu[i]).dot(np.linalg.inv(sigma[i])) *
                             (x-mu[i])).sum(axis=1)) / (np.linalg.det(sigma[i])**0.5) * phi[i]
        w /= w.sum(axis=1)[:, None]
        # M-step
        phi = np.mean(w, axis=0)
        for i in range(K):
            mu[i] = x.T.dot(w[:, i]) / sum(w[:, i])
            sigma[i] = (w[:, i][:, None] * (x-mu[i])
                        ).T.dot(x-mu[i]) / sum(w[:, i])

        it += 1
        prev_ll = ll
        p_xz = np.zeros(w.shape)
        for i in range(K):
            p_xz[:, i] = np.exp(-0.5 * ((x-mu[i]).dot(np.linalg.inv(sigma[i]))
                                * (x-mu[i])).sum(axis=1)) / (np.linalg.det(sigma[i])**0.5) * phi[i]
...
def run_semi_supervised_em(x, x_tilde, z, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        for i in range(K):
            ...
            sigma[i] = ((w[:, i][:, None] * (x-mu[i])).T.dot(x-mu[i]) + alpha * (
                x_tilde_i-mu[i]).T.dot(x_tilde_i-mu[i])) / (sum(w[:, i]) + alpha * sum(z == i))
...
def plot_gmm_preds(x, z, with_supervision, plot_id):
    ...
    plt.title('{} GMM Predictions'.format(
    ...
    file_name = 'p03_pred{}_{}.pdf'.format(
        '_ss' if with_supervision else '', plot_id)
...
def load_gmm_dataset(csv_path):
    ...
    x = np.loadtxt(csv_path, delimiter=',', skiprows=1,
                   usecols=x_cols, dtype=float)
    z = np.loadtxt(csv_path, delimiter=',', skiprows=1,
                   usecols=z_cols, dtype=float)
