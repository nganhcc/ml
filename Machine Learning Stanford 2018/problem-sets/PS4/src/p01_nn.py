    ...
    for x in range (input_width-conv_width+1):
        for y in range(input_height-conv_height+1):
                for k in range(conv_channels):
                    conv_W_grad[k,:, :,:]+= output_grad[k,x,y] * data[:,x:(x+conv_width), y: (y+ conv_height)]
                    data_grad[:,x:(x+conv_width),y:(y+conv_height)]+=output_grad[k,x,y]* conv_W[k,:,:,:]
...
def backward_linear(weights, bias, data, output_grad):
   ...
   # *** START CODE HERE ***
    linear_W_grad=np.outer(data, output_grad)
    linear_data_grad=weights.dot(output_grad)
    linear_bias_grad=output_grad
    return linear_W_grad, linear_bias_grad, linear_data_grad
    # *** END CODE HERE ***
...
def forward_prop(data, labels, params):
    ...
    flattened = first_after_relu.reshape(-1)
