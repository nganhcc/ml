...
def choose_action(state, mdp_data):
    ...
    """
    # *** START CODE HERE ***
    transition_probs_for_state_action0= mdp_data['transition_probs'][state,:,0]
    transition_probs_for_state_action1= mdp_data['transition_probs'][state,:,1]
    value= mdp_data['value']
    future_value_action0=transition_probs_for_state_action0.dot(value)
    future_value_action1=transition_probs_for_state_action1.dot(value)
    if (future_value_action1>=future_value_action0):
        return 1
    return 0
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    num_count=transition_counts.sum(axis=1)
    number_states= transition_counts.shape[0]
    for s in range (number_states):
        for a in range (2):
            if num_count[s,a]==0:
                mdp_data['transition_probs']=transition_counts[s,:,a]/num_count[s,a]
    reward_counts=mdp_data['reward_counts']
    for s in range (number_states):
        nc=reward_counts[s,1]
        if nc!=0:
            mdp_data['reward']=-reward_counts[s,0]/nc
    # *** END CODE HERE ***
